...
export const addGoal = (text, estimatedCompletionDate) => dispatch => {
  ...
  const config = {
    method: 'post',
    url: `https://basrv.dillontarr.com/api/users/add-goal`,
...
export const completeGoal = (goalId, isComplete) => dispatch => {
  ...
  const config = {
    method: 'put',
    url: `https://basrv.dillontarr.com/api/users/modify-goal`,
    headers: { 'x-auth-token': token },
    data: {
      goalId: goalId,
...
export const removeGoal = (goalId) => dispatch => {
  ...
  const config = {
    method: 'delete',
    url: `https://basrv.dillontarr.com/api/users/remove-goal`,
    headers: { 'x-auth-token': token },
    data: {
      goalId: goalId
    }};
