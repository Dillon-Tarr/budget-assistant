...
router.get('/user-details', auth, checkTokenBlacklist, async (req, res) => {
  try {
    ...
    const managedBudgetsWithReminders = await Budget.find( { managers: req.user.loginName, "outgo.doRemind": true }, { outgo: 1 }, function(err, results){ if (err) return res.status(404).send(`The following error occurred when trying to find managed budgets with reminders: ${err}`);} );
